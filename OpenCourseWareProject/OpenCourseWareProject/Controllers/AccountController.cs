...
namespace OpenCourseWareProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var per = db.AspNetUsers.Where(a => a.Admin == null);
                    foreach(AspNetUser item in per)
                    {
                        db.AspNetUsers.Find(item.Id).Admin = "Student";
                    }
                    db.SaveChanges();
        ...
        public ActionResult AccountIndex(string sortOrder = null, string currentFilter = null, int page = 1, int pageSize = 25)
        {
            if(db.AspNetUsers.Find(User.Identity.GetUserId()).Admin == "SuperAdmin")
            {
                ...
                var per = db.AspNetUsers.Where(a => a.Id != id);
                if (!String.IsNullOrEmpty(currentFilter))
                {
                    per = per.Where(p => p.Admin.Contains(currentFilter) || p.Email.Contains(currentFilter));
                }
                switch (sortOrder)
                {
                    case "Email":
                        per = per.OrderBy(x => x.Email);
                        break;
                    case "Email_desc":
                        per = per.OrderByDescending(x => x.Email);
                        break;
                    case "Admin":
                        per = per.OrderBy(x => x.Admin);
                        break;
                    case "Admin_desc":
                        per = per.OrderByDescending(x => x.Admin);
                        break;
                    default:
                        per = per.OrderBy(x => x.Id);
                        break;
                }
                return View(per.ToPagedList(page, pageSize));
            }
        ...
        public ActionResult AccountEdit(string id)
        {
            if (db.AspNetUsers.Find(User.Identity.GetUserId()).Admin == "SuperAdmin")
            {
                ...
                AspNetUser per = db.AspNetUsers.Find(id);
                if(per == null)
                {
                    return HttpNotFound();
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AccountEdit([Bind(Include = "Id,Email,EmailConfirmed,PasswordHash,SecurityStamp,PhoneNumber,PhoneNumberConfirmed,TwoFactorEnabled,LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName,Admin")] AspNetUser Per)
        {
            if(db.AspNetUsers.Find(User.Identity.GetUserId()).Admin == "SuperAdmin")
            {
                ViewBag.Admin = 2;
        ...
        public ActionResult AccountDelete(string id)
        {
            if (db.AspNetUsers.Find(User.Identity.GetUserId()).Admin == "SuperAdmin")
            {
                ...
                var courses = db.Courses.Where(c => c.AspNetUser.Id == id);
                ...
                var curr2 = db.MyCourses.Where(c => c.UserId == id);
                foreach(MyCourses item in curr2)
                {
                    db.MyCourses.Remove(item);
                }
                db.AspNetUsers.Remove(per);
                db.SaveChanges();
                return RedirectToAction("AccountIndex");
            }
            if (db.AspNetUsers.Find(User.Identity.GetUserId()).Admin == "Admin")
